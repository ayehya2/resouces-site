...
import { CheckCircle2, ExternalLink, CheckSquare, Square } from 'lucide-react';
        ...
        <div className="space-y-1 font-mono text-[13px] leading-relaxed">
            {resources.map((resource) => {
                ...
                return (
                    <div
                        key={resource.id}
                        className={`group flex items-start gap-2 py-0.5 transition-colors px-2 -mx-2 ${isSelected ? 'bg-primary/10' : 'hover:bg-muted/50'
                            }`}
                    >
                        <button
                            onClick={() => toggleResourceSelection(resource.id)}
                            className={`mt-1 transition-opacity ${isSelected
                                ? 'text-primary opacity-100'
                                : 'text-muted-foreground opacity-20 group-hover:opacity-100'
                                }`}
                        >
